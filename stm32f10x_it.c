...
* Return         : None
*******************************************************************************/
#define CAP31_PIN               (GPIOA->IDR & GPIO_Pin_0)
#define CAP31_POL_RISING        (TIM2->CCER &= ~TIM_FLAG_CC1)
#define CAP31_POL_FALLING       (TIM2->CCER |= TIM_FLAG_CC1) 
u16 pull;
u16 pull_wi;
u32 cap1_cnt = 0;
...
{
 unsigned int cap1_r_new, cap1_r_old, cap1_f;
  unsigned int pull_period, pull_width;
  
  if((TIM2->SR & TIM_IT_CC1) && (TIM2->DIER & TIM_IT_CC1)){
    cap1_cnt++;                   //overflow counting
    TIM2->SR = (u16) ~TIM_IT_CC1; //flag clear
    if(CAP31_PIN){
      cap1_r_new = TIM2->CCR1;
      pull_period = (u32) (cap1_r_new - cap1_r_old);
      cap1_r_old = cap1_r_new;                      //  _|-------|____|--------|__
      CAP31_POL_FALLING;                            //   |<-From here |<- to here
      pull =  pull_period;                          //   |<-- pull -->|         
    }
    else{                        //For duty Cycle.    _|--------|__|---------|__
      cap1_f = TIM2->CCR1;       //                    |<-This->|  |<-This ->|         
      pull_width = (u32) (cap1_f - cap1_r_new);
      CAP31_POL_RISING;
      pull_wi =  pull_width;
    }
  }
